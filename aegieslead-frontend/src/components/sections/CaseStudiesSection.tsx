...
export const CaseStudiesSection: React.FC<Props> = ({ settings }) => {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {cards.map((card, idx) => (
            <div
              key={idx}
              className="flex flex-col justify-between rounded-2xl bg-white border border-slate-200/80 p-8 shadow-sm hover:shadow-xl hover:border-blue-300 transition-all group"
            >
              ...
              <div className="pt-6 mt-6 border-t border-slate-100">
                <a
                  href={card.link_url || '#case'}
                  className="inline-flex items-center gap-2 text-sm font-bold text-blue-700 group-hover:text-blue-800 transition-colors"
                >
                  {card.link_text || 'Read Case Study'}
